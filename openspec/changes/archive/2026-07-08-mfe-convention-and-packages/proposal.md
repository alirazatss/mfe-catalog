...
Following the Turborepo setup, we need to adopt the `apps/mfe-*` naming convention for micro-frontends and create the foundational packages (`monorepo-tools` and `remote-config`) that will enable auto-discovery and config generation.

## What Changes

- Rename `apps/remote-widget/` to `apps/mfe-widget/` to follow convention
