...
- Create **convention-based remote registration** (any `apps/mfes/mfe-*` automatically becomes a remote)
...
- `apps/shells/website/vite.config.ts` - Remove hardcoded remotes, add config generation logic
- `apps/shells/website/src/config/remotes.ts` - Rewrite to load from generated `public/remotes.config.json`
- `apps/shells/website/src/RemoteWidgetLoader.ts` - Use dynamic loader to load any discovered remote
- `apps/mfes/remote-widget/` - Rename to `apps/mfes/mfe-widget/` to follow naming convention
...
- `apps/shells/website/public/remotes.config.json` - Auto-generated at build time (gitignored)
...
- Convention over configuration (apps/mfes/mfe-\* pattern)
- Generated config committed or deployed with host
- Optional remote cache (Vercel, self-hosted, or local-only)

### Deployment Changes

- Adding new remote: Create `apps/mfes/mfe-{name}/` - no host changes needed
