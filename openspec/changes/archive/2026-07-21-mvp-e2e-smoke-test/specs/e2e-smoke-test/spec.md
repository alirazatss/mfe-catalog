...
- **Test execution**: ~10-30 seconds per run
- **Flakiness risk**: Low (uses mock backend, no external dependencies)
- **CI/CD impact**: Can run on every commit (fast)
- **Tools required**: Playwright, MSW or Node http server
- **Maintenance**: Low (mock endpoints unlikely to change)
- **Success criterion**: Test passes = MVP journey validated end-to-end
