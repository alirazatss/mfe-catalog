...
- [x] 1.1 Add slot elements (`header-slot`, `sidebar-slot`, `main-slot`, `footer-slot`) inside `#app` in `apps/website/index.html` with `data-slot` attributes
...
- [x] 1.5 Rewrite `apps/website/src/style.css` to contain only CSS grid layout targeting slot IDs; remove all color, typography, and component styling
...
- [x] 2.5 Update `apps/website/public/remotes.config.json` to the new manifest format with `mfe-widget` under `features["/widget"]`
...
- [x] 3.1 Rewrite `apps/website/src/main.ts` as the vanilla bootstrap entry (no React import)
...
- [x] 5.1 Delete `apps/website/src/providers/AuthProvider.tsx` and any imports of it
- [x] 5.2 Delete `apps/website/src/components/LoginPage.tsx`
- [x] 5.3 Delete `apps/website/src/components/Layout.tsx`
- [x] 5.4 Delete `apps/website/src/components/ProtectedRoute.tsx`
- [x] 5.5 Delete `apps/website/src/components/HomePage.tsx`
- [x] 5.6 Delete `apps/website/src/components/NotFoundPage.tsx`
- [x] 5.7 Delete `apps/website/src/components/NavigationEventListener.tsx`
- [x] 5.8 Delete `apps/website/src/App.tsx` (or reduce to a re-export stub if referenced anywhere)
- [x] 5.9 Delete `apps/website/src/main.tsx` if replaced by `main.ts`
- [x] 5.10 Delete corresponding test files (`Layout.test.tsx`, `ProtectedRoute.test.tsx`) that cover removed components
- [x] 5.11 Remove `react-router` and any React DOM rendering dependencies from `apps/website/package.json` (keep `react`/`react-dom` only if still needed by other tooling; otherwise remove)
- [x] 5.12 Remove unused imports and dead files under `apps/website/src/`
...
- [x] 6.8 Add a build-time check (script or vitest) that fails if runtime shell code under `apps/website/src/**/*.{ts,js}` exceeds 250 non-blank, non-comment lines (excluding `*.d.ts` and `*.test.*`)
...
- [x] 7.2 Wire `check-shell-size` into `apps/website/package.json` prebuild step
...
- [x] 8.7 Confirm no lingering references to deleted components remain (`grep -R "AuthProvider\|LoginPage\|ProtectedRoute\|NavigationEventListener" apps/website/src`)
