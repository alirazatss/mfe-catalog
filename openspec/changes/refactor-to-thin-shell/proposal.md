...
- `apps/shells/website/src/App.tsx` — reduced to bootstrap coordinator
- `apps/shells/website/src/main.tsx` — becomes primary entry point (~50 lines)
- `apps/shells/website/src/providers/AuthProvider.tsx` — logic preserved, React wrapper removed
- `apps/shells/website/src/components/Layout.tsx` — removed (migrates to `mfe-header` in future change)
- `apps/shells/website/src/components/LoginPage.tsx` — removed (migrates to `@mfe-runtine/auth-ui` in future change)
- `apps/shells/website/src/components/ProtectedRoute.tsx` — removed (auth check moves to bootstrap)
- `apps/shells/website/index.html` — adds slot placeholders and base grid CSS
- `apps/shells/website/src/style.css` — reduced to layout-only CSS

**Affected dependencies:**

- `apps/shells/website/package.json` — remove `react-router` from shell dependencies (still used by MFEs)
- Existing MFE (`apps/mfes/mfe-widget`) continues working unchanged

**Affected tests:**

- Shell tests (`apps/shells/website/src/**/*.test.tsx`) for removed components will be deleted
